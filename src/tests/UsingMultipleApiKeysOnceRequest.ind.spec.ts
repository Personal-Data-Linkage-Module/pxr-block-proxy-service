...
import {
    StubService,
    StubAbnormalService,
    StubAccessControlMultiKeyService as StubAccessControlService,
    StubOperatorService,
    StubCatalogService,
    StubReverseProxyAPI
} from './StubServer';
...
describe('PXR-Block-Proxy Service API', () => {
    ...
    beforeAll(async () => {
        ...
        _mockOperatorService = new StubOperatorService();
    ...
    describe('プロキシーAPI (GET|POST|PUT|DELETE): ' + baseURI, () => {
        ...
        test('異常系: GET（サービスが起動していない）', async () => {
            ...
            expect(JSON.stringify(response.body)).toBe(JSON.stringify([
                { status: 500, message: '対象のサービスとの接続に失敗しました' }, { status: 500, message: '対象のサービスとの接続に失敗しました' }, { status: 500, message: '対象のサービスとの接続に失敗しました' }
            ]));
